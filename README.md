...
- **Coverage counts emitted claims, not everything the answer says.** `coverage_ratio` is `checkable / claims the model emitted`. An assertion made only in the answer prose, never emitted as a claim, is not UNCHECKABLE — it is *uncounted*, and invisible to the denominator. The shipped demo shows this: its answer states a total of $45300 and a monthly average of $15000, while the claims assert 46300 and 15367 — so the two numbers a reader actually sees were checked by nothing, even though `coverage_ratio` reads 1.000. Treat coverage as a floor on what was examined, never as a score for the answer.
