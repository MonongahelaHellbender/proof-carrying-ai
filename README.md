...
- **Coverage counts emitted claims, not everything the answer says.** `coverage_ratio` is `checkable / claims the model emitted`. An assertion made only in the answer prose, never emitted as a claim, is not UNCHECKABLE — it is *uncounted*. Because that gap was invisible, it is now measured separately as **answer coverage**: every number the answer states that is not asserted by a claim, not inside a VERIFIED quote, and not given in the facts. The shipped demo reports **3 of 7 unbacked** — its answer states $45300, $15000 and 30, none of which carry a receipt, while `coverage_ratio` reads 1.000. Note that 30 *does* appear in the source, but not inside the span the model actually quoted: the receipt is the quote, not the corpus. **Measuring the gap is not closing it.** Treat coverage as a floor on what was examined, never as a score for the answer.
- **Coverage measures checkability, not importance.** Nothing stops a model from inflating the ratio with trivially-checkable claims while leaving the load-bearing assertion unstated. Not mitigated.
